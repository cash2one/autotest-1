...
def new_task(request):
    ...
    taskcases_union = TESTCASE.objects.raw('SELECT DISTINCT(testsuitID) testsuitID,Testsuit_name,id FROM `TESTCASE` WHERE PLATFORM_ID_id=1 AND status=0 GROUP BY testsuitID')
    taskcases_dict = TESTCASE.objects.raw('SELECT DISTINCT(testsuitID) testsuitID,Testsuit_name,id FROM `TESTCASE` WHERE PLATFORM_ID_id=2  AND status=0 GROUP BY testsuitID')
        ...
        if  request.is_ajax():
            if request.method == "POST":
                ...
                if platform == "Union":
                    ...
                    env = ENV.objects.create(PLATFORM_ID=PLATFORM[0],LDAP=usr,TIME=timenow,STATUS="0",IMPR_SVN=impr_svn,RESIN_SVN=resin_svn,IMPR_PORT="0",RESIN_PORT="1",EMMA_PORT="1")
